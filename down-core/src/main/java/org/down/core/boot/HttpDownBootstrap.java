...
import org.down.core.constant.HttpDownStatus;
import org.down.core.dispatch.HttpDownCallback;
import org.down.core.entity.*;
import org.down.core.exception.*;
import org.down.core.handle.DownTimeoutHandler;
import org.down.core.proxy.FileUtil;
import org.down.core.proxy.OsUtil;
...
import org.down.core.utils.ProtoUtil;
...
/**
 * <p>{@link HttpDownBootstrap}</p>
 * http 下载器
 * @author 白菜
 * @since Created in 2019/12/3 15:10
 */
@Slf4j
@Data
...
public class HttpDownBootstrap implements Serializable {
    ...
    private static final long serialVersionUID = 1557848278544923713L;

    private static final long BASIC_SIZE = 1024 * 1024L;
    private static final double BASIC_TIME_NANO =(double) TimeUnit.SECONDS.toNanos(1);
    ...
    /**
     * 开始下载
     * @param isResume 是否重新下载
     */
    private void start(boolean isResume) {
        long startTime = 0;
        long lastPauseTime = 0;
        if (Objects.nonNull(taskInfo) && HttpDownStatus.WAIT == taskInfo.getStatus()) {
        ...
        try {
            ...
            if (downFile.exists()) {
                ...
                if (isResume && downFile.delete()) {
                    log.info("无进度记录，删除相同文件重新下载:{}", response.getFileName());
            ...
            try {
                ...
                if (response.isSupportRange()) {
                    ...
                    if (OsUtil.isUnix() || "NTFS".equalsIgnoreCase(fileSystemType)
                            || "UFS".equalsIgnoreCase(fileSystemType) || "APFS".equalsIgnoreCase(fileSystemType)) {
            ...
            if (Objects.nonNull(progressThread)) {
     ...
     * 生成下载分段信息
     */
    private void buildChunkInfoList() {
        ...
        if (response.getTotalSize() > 0) {
            ...
            for (int i = 0; i < downConfig.getConnections(); i++) {
                ...
                // 最后一个连接去下载多出来的字节
                if (i == downConfig.getConnections() - 1) {
    ...
    class ProgressThread extends Thread {
        ...
        public void run() {
            final int period = 1;
            while (run) {
                if (HttpDownStatus.DONE != taskInfo.getStatus()) {
                    for (ChunkInfo chunkInfo : taskInfo.getChunkInfoList()) {
                        synchronized (ChunkInfo.class) {
                            if (HttpDownStatus.DONE != chunkInfo.getStatus()) {
                    ...
                    taskInfo.setSpeed(taskInfo.getChunkInfoList()
                            .stream()
                            .filter(chunkInfo -> HttpDownStatus.DONE != chunkInfo.getStatus())
    ...
    private void commonStart() {
        ...
        if (Objects.isNull(progressThread)) {
    ...
    private void connect(ConnectInfo connectInfo) {
        ...
        log.debug("开始下载：" + connectInfo);
        Bootstrap bootstrap = new Bootstrap()
                .channel(NioSocketChannel.class)
                .group(loopGroup)
                .handler(new HttpDownInitializer(requestProto.isSsl(), connectInfo));
        if (proxyConfig != null) {
            //代理服务器解析DNS和连接
        ...
        // 重置最后下载时间
        connectInfo.setConnectChannel(channelFuture.channel());
        channelFuture.addListener((ChannelFutureListener) future -> {
            if (future.isSuccess()) {
                log.debug("连接成功：" + connectInfo);
                if (response.isSupportRange()) {
    ...
    public void pause() {
        synchronized (TaskInfo.class) {
            if (taskInfo.getStatus() == HttpDownStatus.PAUSE || taskInfo.getStatus() == HttpDownStatus.DONE) {
                return;
            }
            ...
            for (ConnectInfo connectInfo : taskInfo.getConnectInfoList()) {
                if (connectInfo.getStatus() != HttpDownStatus.DONE) {
                    connectInfo.setStatus(HttpDownStatus.PAUSE);
                }
            }
            close();
        }
     ...
     */
    public void resume() {
        if (Objects.isNull(taskInfo) || Objects.isNull(taskInfo.getChunkInfoList())) {
            // 下载进度信息不存在
            start(true);
            return;
        }
    ...
    private void close() {
        synchronized (TaskInfo.class) {
            if (Objects.nonNull(taskInfo.getConnectInfoList())) {
                for (ConnectInfo connectInfo : taskInfo.getConnectInfoList()) {
                    close(connectInfo);
                }
            }
    ...
    private class HttpDownInitializer extends ChannelInitializer {

        private boolean isSsl;
        private ConnectInfo connectInfo;

        private HttpDownInitializer(boolean isSsl, ConnectInfo connectInfo) {
            this.isSsl = isSsl;
        ...
        protected void initChannel(Channel channel) throws Exception {
            ...
            if (isSsl) {
                ProtoUtil.RequestProto requestProto = request.getRequestProto();
                pipeline.addLast(HttpDownUtil.getSslContext().newHandler(channel.alloc(), requestProto.getHost(), requestProto.getPort()));
            }
            pipeline.addLast("downTimeout", new DownTimeoutHandler(downConfig.getTimeout()));
            ...
            pipeline.addLast(new ChannelInboundHandlerAdapter() {
                            ...
                            synchronized (TaskInfo.class) {
                                ...
                                if (downConfig.getSpeedLimit() > 0) {
                                    ...
                                    if (taskInfo.getLastStartTime() <= 0) {
                                        ...
                                        double limitSpeed = downConfig.getSpeedLimit() / BASIC_TIME_NANO;
                                        long sleepTime = (long) ((speed - limitSpeed) * useTime);
                                        if (sleepTime > 0) {
                                            TimeUnit.NANOSECONDS.sleep(sleepTime);
                                            // 刷新其他连接的lastReadTime
                                            for (ConnectInfo info : taskInfo.getConnectInfoList()) {
                                                if (Objects.nonNull(info.getConnectChannel()) && info.getConnectChannel() != channel) {
                                                    DownTimeoutHandler downTimeout = (DownTimeoutHandler) info.getConnectChannel().pipeline().get("downTimeout");
                                                    if (Objects.nonNull(downTimeout)) {
                                ...
                                if (connectInfo.getDownSize() >= connectInfo.getTotalSize()) {
                                    log.debug("连接下载完成：" + connectInfo + "\t" + chunkInfo);
                                    normalClose = true;
                                    close(connectInfo);
                                    // 判断分段是否下载完成
                                    if (isChunkDownDone(httpContent)) {
                                        log.debug("分段下载完成：" + connectInfo + "\t" + chunkInfo);
                                        // chunked编码最后更新文件大小
                                        if (!response.isSupportRange()) {
                                        ...
                                        // 所有分段都下载完成
                                        if (taskInfo.getChunkInfoList()
                                                .stream()
                                                .allMatch(chunk -> chunk.getStatus() == HttpDownStatus.DONE)) {
                                            // 文件下载完成回调
                                            log.debug("任务下载完成：" + chunkInfo);
                                    ...
                                    ChunkInfo supportChunk = taskInfo.getChunkInfoList()
                                            ...
                                            .orElse(null);
                                    if (Objects.isNull(supportChunk)) {
                                        connectInfo.setStatus(HttpDownStatus.DONE);
                                        return;
                                    }
                                    ConnectInfo maxConnect = taskInfo.getConnectInfoList()
                                            .stream()
                                            .filter(connect -> connect.getChunkIndex() == supportChunk.getIndex() && connect.getTotalSize() - connect.getDownSize() >= BASIC_SIZE)
                                            .max((c1, c2) -> (int) (c1.getStartPosition() - c2.getStartPosition()))
                                            .orElse(null);
                                    if (Objects.isNull(maxConnect)) {
                                        connectInfo.setStatus(HttpDownStatus.DONE);
                                        return;
                                    }
                                    // 把这个分段最后一个下载连接分成两个
                                    ...
                                    log.debug("支持下载：" + connectInfo + "\t" + chunkInfo);
                                    reConnect(connectInfo, true);
                                }
                            }
                        } else {
                            HttpResponse httpResponse = (HttpResponse) msg;
                            int responseCode = httpResponse.status().code();
                            if (responseCode < 200 || responseCode >= 300) {
                                log.warn("响应状态码异常：" + responseCode + "\t" + connectInfo);
                                // 处理重定向链接
                                ...
                                normalClose = true;
                                close(connectInfo);
                                channel.eventLoop().schedule(() -> reConnect(connectInfo), 5, TimeUnit.SECONDS);
                                return;
                            }
                            log.debug("下载响应：" + connectInfo);
                ...
                private boolean isChunkDownDone(HttpContent httpContent) {
                    if (response.isSupportRange()) {
                        return chunkInfo.getDownSize() >= chunkInfo.getTotalSize();
                    } else {
                ...
                public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
                    if (!(cause instanceof IOException) && !(cause instanceof ReadTimeoutException)) {
                        log.error("down onChunkError:", cause);
                    }
                    normalClose = true;
                    close(connectInfo);
                    reConnect(connectInfo);
                ...
                public void channelUnregistered(ChannelHandlerContext ctx) throws Exception {
                    ...
                    if (!normalClose
                        ...
                        log.debug("channelUnregistered:" + connectInfo + "\t" + chunkInfo);
                        close(connectInfo);
                        reConnect(connectInfo);
                    }
                }
    ...
    public static UrlHttpDownBootstrapBuilder builder() {
        return new UrlHttpDownBootstrapBuilder();
    }

    public static UrlHttpDownBootstrapBuilder builder(String url, Map<String, String> heads, String body) {
        return new UrlHttpDownBootstrapBuilder().setUrl(url).setHeads(heads).setBody(body).setMethod(null);
    }
}
